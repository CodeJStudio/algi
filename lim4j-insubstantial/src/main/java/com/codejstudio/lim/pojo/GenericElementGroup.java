...
public class GenericElementGroup extends GenericElement implements IGroupable<GenericElement> {
	...
	@Override
	public List<GenericElement> getInnerGroupList() throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			return null;
		}

		List<GenericElement> l = CollectionUtil.generateNewList();
		for (BaseElement be : this.innerGroupList) {
			l.add((GenericElement) super.getInnerElementDelegate(be));
		}
	...
	@Override
	public boolean replaceGroupElement(final BaseElement replacee, final GenericElement replacer) 
			throws LIMException {
		return (Objects.equals(replacee, replacer) 
						|| (replacee != null && !(replacee instanceof GenericElement) 
