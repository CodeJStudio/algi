...
public class EntityGroup extends Entity implements IGroupable<Entity> {
	...
	@Override
	public List<Entity> getInnerGroupList() throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			return null;
		}

		List<Entity> l = CollectionUtil.generateNewList();
		for (BaseElement be : this.innerGroupList) {
			l.add((Entity) super.getInnerElementDelegate(be));
		}
	...
	@Override
	public boolean replaceGroupElement(final BaseElement replacee, final Entity replacer) 
			throws LIMException {
		return (Objects.equals(replacee, replacer) || (replacee != null && !(replacee instanceof Entity) 
