...
public class OwnableInformationElementGroup 
	...
	@Override
	public List<OwnableInformationElement> getInnerGroupList() throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			return null;
		}

		List<OwnableInformationElement> l = CollectionUtil.generateNewList();
		for (BaseElement be : this.innerGroupList) {
			l.add((OwnableInformationElement) super.getInnerElementDelegate(be));
		}
	...
	@Override
	public boolean replaceGroupElement(final BaseElement replacee, final OwnableInformationElement replacer) 
			throws LIMException {
		return (Objects.equals(replacee, replacer) 
						|| (replacee != null && !(replacee instanceof OwnableInformationElement) 
