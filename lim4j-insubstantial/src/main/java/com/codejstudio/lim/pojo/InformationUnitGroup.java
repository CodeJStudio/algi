...
public class InformationUnitGroup extends InformationUnit implements IGroupable<InformationUnit> {
	...
	@Override
	public List<InformationUnit> getInnerGroupList() throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			return null;
		}

		List<InformationUnit> l = CollectionUtil.generateNewList();
		for (BaseElement be : this.innerGroupList) {
			l.add((InformationUnit) super.getInnerElementDelegate(be));
		}
	...
	@Override
	public boolean replaceGroupElement(final BaseElement replacee, final InformationUnit replacer) 
			throws LIMException {
		return (Objects.equals(replacee, replacer) 
						|| (replacee != null && !(replacee instanceof InformationUnit) 
