...
public class CommentGroup extends Comment implements IGroupable<Comment> {
	...
	@Override
	public List<Comment> getInnerGroupList() throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			return null;
		}

		List<Comment> l = CollectionUtil.generateNewList();
		for (BaseElement be : this.innerGroupList) {
			l.add((Comment) super.getInnerElementDelegate(be));
		}
	...
	@Override
	public boolean replaceGroupElement(final BaseElement replacee, final Comment replacer) 
			throws LIMException {
		return (Objects.equals(replacee, replacer) || (replacee != null && !(replacee instanceof Comment) 
